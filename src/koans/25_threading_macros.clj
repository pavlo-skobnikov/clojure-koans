...
(defn function-that-takes-a-map [map a b] (get map :a))

(defn function-that-takes-a-coll [a b coll] (map :a coll))
...
(meditations
  "We can use thread first for more readable sequential operations"
    (= {:a 1}
       (-> {}
           (assoc :a 1)))
  "Consider also the case of strings" (= "Hello world, and moon, and stars"
                                         (-> "Hello world"
                                             (str ", and moon")
                                             (str ", and stars")))
  ...
  "We can also thread last using ->>" (= [2 3 4]
                                         (->> [1 2 3]
                                              (map inc)))
